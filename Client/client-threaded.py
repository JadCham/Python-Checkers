...
class Client(threading.Thread):
    # Code to run on Exit
    def __init__(self, clientsocket, user, password, data):
        self.username = user
    ...
    def client(self):
        ...
        while True:
            ...
            count = 0
            friend = "jadtest"
            # self.sock.sendall("adduser:/jadtest1|p@ssw0rd|Jad Chamoun")
            # self.sock.sendall("msg:/%s|%s" % (self.username, friend))
            # self.sock.sendall("afriend:/%s|%s" % (username, friend))
            # self.sock.sendall("greq:/%s|%s" % (self.username, friend))
            ...
            while True:
                if self.data != olddata:
                    ...
                    try:
                        d = "%s" % self.data
                        self.sock.sendall(self.data.encode('ascii'))
                        olddata = self.data
                    except socket.error:
                        print ("Error sending data ")
                try:
                    req = self.sock.recv(1024).decode('ascii')
                    if req != Response:
                        print (req)
